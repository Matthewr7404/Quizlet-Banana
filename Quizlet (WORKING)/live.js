class QuizletBanana {
  ...
  async StartWatchQuestions() {
    if (document.querySelector(".StudentPrompt") === null) {
      setTimeout(function () {
        window.QuizletBanana.StartWatchQuestions();
      }, 100);
    } else {
      window.QuizletBanana.WatchQuestions();
    }
  }
  ...
  HighlightCorrectAnswer() {
    try {
      if (document.querySelector(".StudentPrompt-text div") !== null) {
        ...
        let answers = window.QuizletBanana.terms
          .map((t) =>
...
window.QuizletBanana = new QuizletBanana();

chrome.runtime.onMessage.addListener(function (msg) {
  if (msg.gamePin && !window.QuizletBanana.gotPin)
    window.QuizletBanana.GetTerms(msg.gamePin);
});
